...
"""
        ...
        if "detalles_por_zona" in self.context_data:
            ...
            for zona, datos in self.context_data["detalles_por_zona"].items():
                context_str += f"\n{zona}:\n"
                context_str += f"  Total delitos: {datos['total_delitos']:,}\n"
                context_str += f"  Predichos: {datos['delitos_predichos']:,}\n"
                context_str += f"  Tipos:\n"
                for tipo, cant in datos['tipos_delito'].items():
...
"""
            ...
            context_str += "\nHORARIOS CON MÁS RIESGO ALTO:\n"
            for horario, cant in pb['horarios_mas_riesgo'].items():
                context_str += f"• {horario}: {cant} casos\n"
